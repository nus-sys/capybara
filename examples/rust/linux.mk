...
#tcp-migration,
all-examples:
	$(CARGO) build --example tcp-server $(CARGO_FEATURES) $(CARGO_FLAGS) $(EXAMPLE_FEATURES)
...
# $(CARGO) build --example capybara-switch $(CARGO_FEATURES) $(CARGO_FLAGS) --features=capybara-switch,capy-profile
# $(CARGO) build --example proxy-server-fe $(CARGO_FEATURES) $(CARGO_FLAGS) $(EXAMPLE_FEATURES)
# $(CARGO) build --example proxy-server-be $(CARGO_FEATURES) $(CARGO_FLAGS) $(EXAMPLE_FEATURES),capy-profile,capy-time-log

clean:
	@rm -rf $(BINDIR)/examples/rust/tcp-server.$(EXEC_SUFFIX)

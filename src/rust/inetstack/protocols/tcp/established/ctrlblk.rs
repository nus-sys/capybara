...
impl ControlBlock {
    ...
    pub fn receive(&self, mut header: &mut TcpHeader, mut data: Buffer) {
        ...
        if seg_start != receive_next {
            ...
            if seg_start < receive_next {
                ...
                if seg_end < receive_next {
                    ...
                    //
                    if !header.rst {
                        capy_log!("Duplicate pkt received. ACK and Drop.");
                        self.send_ack();
                    }
                    return;
                } else {
                    ...
                    capy_log!("Partially duplicate pkt received. Cut duplicate part.");
                ...
                if seg_start >= after_receive_window {
                    ...
                    //
                    if !header.rst {
                        capy_log!("Segment outside window received. ACK and Drop.");
                        self.send_ack();
                    }
                    return;
                }
        ...
        // Check the SYN bit.
        if header.syn {
            capy_log!("SYN on established conn => Error");
        ...
        if !header.ack {
            ...
            capy_log!("non-ACK on established conn => Error");
            return;
        }
        ...
        if send_unacknowledged < header.ack_num {
            if header.ack_num <= send_next {
                ...
                if header.ack_num == send_next {
                    ...
                    capy_log!("ALL sent data ACK'ed => Retransmit OFF");
        ...
        if header.fin {
            ...
            capy_log!("Sending ACK for the FIN");
            self.send_ack();
            return;
        }
...
pub mod state {
    ...
    use crate::{
        ...
        runtime::{
            memory::Buffer,
            network::{NetworkRuntime, config::TcpConfig},
            timer::TimerRc,
    ...
    impl From<&ControlBlock> for ControlBlockState {
        fn from(ControlBlock {
            local,
            remote,
            receive_buffer_size,
            window_scale,
    ...
    #[cfg(test)]
    pub mod test {
        use std::{collections::VecDeque, net::{SocketAddrV4, Ipv4Addr}};

        use crate::{inetstack::protocols::tcp::SeqNumber, runtime::memory::{Buffer, DataBuffer}};

        use super::{ControlBlockState, ReceiverState};

        fn get_buf() -> Buffer {
            Buffer::Heap(DataBuffer::from_slice(&vec![1; 64]))
        }

        pub fn get_state() -> ControlBlockState {
            ...
            ControlBlockState {
                local: SocketAddrV4::new(Ipv4Addr::LOCALHOST, 10000),
                remote: SocketAddrV4::new(Ipv4Addr::LOCALHOST, 10000),

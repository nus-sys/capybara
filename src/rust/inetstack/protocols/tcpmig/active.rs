...
pub struct ActiveMigration<const N: usize> {
    ...
    pub recv_queue: VecDeque<(Ipv4Header, TcpHeader, DemiBuffer)>,
...
impl<const N: usize> ActiveMigration<N> {
    pub fn new(
        rt: Rc<dyn NetworkRuntime<N>>,
        local_ipv4_addr: Ipv4Addr,
        local_link_addr: MacAddress,
    ...
    pub fn process_packet(&mut self, _ipv4_hdr: &Ipv4Header, hdr: TcpMigHeader, buf: DemiBuffer) -> Result<MigrationRequestStatus, Fail> {
        #[inline]
    ...
    pub fn send_connection_state(&mut self, state: TcpState) {
        ...
        let buf = DemiBuffer::from_slice(&buf).expect("Exceeded DemiBuffer size limit"); 
